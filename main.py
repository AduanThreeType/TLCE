...
trtkc = TRTKC(train_real_ngh_finder[0], test_real_feat, num_layers=NUM_LAYER, use_time=USE_TIME,
              agg_method=AGG_METHOD, attn_mode=ATTN_MODE, seq_len=SEQ_LEN, n_head=NUM_HEADS, drop_out=DROP_OUT)
trtkc = trtkc.to(device)

optimizer = torch.optim.Adam(list(trtkc.parameters()) + list(MLP_model.parameters()), LEARNING_RATE)
...
for epoch in range(NUM_EPOCH):
    ...
    for j in tqdm(range(len(train_real_ts_l))):
        trtkc.ngh_finder = train_real_ngh_finder[j]
        m_loss = []
        ...
        for k in range(num_train_batch):
            ...
            src_embed = trtkc.tem_conv(src_l_cut, ts_l_cut, NUM_LAYER, num_neighbors=NUM_NEIGHBORS)
            ...
            loss.backward()
            #梯度剪裁
            nn.utils.clip_grad_norm_(list(trtkc.parameters()) + list(MLP_model.parameters()), max_norm=1.0)
            optimizer.step()
            scheduler.step()
...
torch.save(trtkc.state_dict(), './saved_models/model_tatkc.pth')
print("tatkc_model save over")
#---------------------------------------- 训练测试集 -----------------------------------------
real_data_start_time = time.time()
eval_real_data('test for real data', trtkc, MLP_model, test_real_ngh_finder,
               nodeList_test_real, test_real_ts_list, test_label_l_real)
