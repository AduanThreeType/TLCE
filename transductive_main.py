...
parser.add_argument('-d', '--data', type=str, help='data sources to use', default='edit-tgwiktionary')
...
def eval_real_data(hint, tgan, lr_model, sampler, src, ts, label):
    # eval_real_data('test for real data', tatkc, MLP_model, test_real_ngh_finder,
    ...
    with torch.no_grad():
        ...
        mae_v = MAE(test_pred_tbc_list,label)
        mse_v = MSE(test_pred_tbc_list,label)
...
# print('./data/test/Real/processed/ml_{}_node.npy'.format(DATA))
# Load data
...
#训练最大时间戳集合，每个最大长度  测试时间戳集合,每个最大长度
...
tatkc = TRTKC(test_real_ngh_finder, test_real_feat, num_layers=NUM_LAYER, use_time=USE_TIME,
...
num_train_instance = len(test_real_node)
...
for epoch in range(NUM_EPOCH):
    ...
    for k in range(num_train_batch):
        ...
        e_idx = min(num_train_instance, s_idx + TRAIN_BATCH_SIZE)
                    #索引切片
        src_l_cut = np.array(test_real_node[s_idx:e_idx])
                    #tkc值切片
        label_l_cut = test_label_l_real[s_idx:e_idx]
        ts_l_cut = test_real_ts_l[s_idx:e_idx]
        optimizer.zero_grad()
        ...
        loss = MSEloss(pred_bc, true_label)
...
train_end = time.time()
print('train end, train_time={}'.format(train_end - start_time))
torch.save(MLP_model.state_dict(), f'./saved_models/self_learning_{DATA}_model_MLP_epoch2.pth')
print("MLP_model save over")
torch.save(tatkc.state_dict(), f'./saved_models/self_learning_{DATA}_model_tatkc_epoch2.pth')
...
# torch.save(MLP_model.state_dict(), f'./saved_models/transductice_{DATA}_model_MLP.pth')
# print("MLP_model save over")
# torch.save(tatkc.state_dict(), './saved_models/transductice_{DATA}_model_tatkc.pth')
# print("tatkc_model save over")
# #---------------------------------------- 训练测试集 -----------------------------------------
...
# logger.info('Test real data statistics: nodes -- time:{}'
#             .format(real_data_end_time - real_data_start_time))
